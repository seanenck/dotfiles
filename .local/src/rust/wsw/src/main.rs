...
impl Context {
    ...
    fn to_server(&self) -> (String, String, String) {
        (
    ...
    fn from_server(configs: String, cache: String, bind: String) -> Context {
        Context {
            configs,
            cache,
            bind,
...
fn change_network(ctx: &Context, profile: String) {
    ...
    match p {
        Some(obj) => {
            update(ctx.configs.to_string(), ctx.cache.to_string(), obj);
        }
        None => {
...
fn run_monitor(config: String, cache: String, bind: String) {
    let ctx = Context::from_server(config, cache, bind);
...
fn daemonize(ctx: &Context) {
    ...
    let monitor = thread::spawn(move || {
        run_monitor(server.0, server.1, server.2);
    });
